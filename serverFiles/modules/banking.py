        ...
        if os.path.exists("moduleFiles/bank/%s"%(sId)) and os.path.exists("moduleFiles/bank/%s"%(rId)): #if both accounts exist
            ...
            if details["key"] == sKey: #if key is valid
                if details["balance"] >= amount or details["admin"]: #if has money
                    ...
                    with open("moduleFiles/bank/%s"%(sId),"w") as f:import standard, asyncio, os, json, random
...
async def WSAPIcreateBank(websocket,deviceName,modules): #create the bank account
...
async def WSAPIaccountLoad(websocket,deviceName,modules): #loading account name and balance
...
async def WSAPIaccountNameChange(websocket,deviceName,modules): #chaning account name
    message = json.loads(await websocket.recv())
    id = message["id"] #account id
    key = message["key"] #account key
...
async def WSAPItransferMoney(websocket,deviceName,modules): #transfer money between accounts
    message = json.loads(await websocket.recv())
